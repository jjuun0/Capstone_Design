...
import PIL
from ii2s.utils import data_utils
import torchvision.transforms as transforms
import os
from ii2s.utils.shape_predictor import align_face
import sys
...
class ImagesDataset(Dataset):
    ...
    def __init__(self, opts, image_path=None):
        ...
        if type(image_path) == PIL.Image.Image:
            self.image_paths = image_path
    ...
    def __len__(self):
        if type(self.image_paths) == PIL.Image.Image:
            return 1
        return len(self.image_paths)

    def __getitem__(self, index):
        if type(self.image_paths) == PIL.Image.Image:
            im_H = self.image_paths
        # else:
