...
def get_landmark(filepath, predictor):
    ...
    img = dlib.load_rgb_image(filepath)  # 이미지 읽기, ndarray(h, w, c)
    # dets = detector(img, 1)  # upsample 개수 1, 빠른 속도를 원하면 0
    dets = detector(img, 1)  # upsample 개수 1, 빠른 속도를 원하면 0
    assert len(dets) > 0, "Face not detected, try another face image"

    for k, d in enumerate(dets):  # dets: [left_top, right_bottom 좌표]
        shape = predictor(img, d)  # 랜드마크 검출
...
def align_face(filepath, output_size=1024, transform_size=4096, enable_padding=True):
    ...
    """
    :param filepath: ndarray
    :return: PIL Image
    """
    ensure_checkpoint_exists("models/dlibshape_predictor_68_face_landmarks.dat")
    predictor = dlib.shape_predictor("models/dlibshape_predictor_68_face_landmarks.dat")  # 얼굴 랜드마크를 불러오기 위한 모델 가져오기
    lm = get_landmark(filepath, predictor)  # 얼굴의 랜드마크 얻기
